...
from model import SRCNN, PowderUNet, FNO2d, EDSR
...
def fetch_mp_ids(api_key, limit=80, num_elements=(1, 3), nsites_max=20,
    ...
    """自动从MP获取候选ID，标准化元素名后过滤黑名单；过采样以便后面凑够有效样本。"""
...
def inverse_scale(arr, scaler):
    """
    Robust inverse for MinMaxScaler:
    - Accepts any shape whose last dims multiply to n_features (e.g., (B,1,100,200)).
    - Flattens to (N, n_features), applies scaler.inverse_transform, then reshapes back.
    """
    import numpy as np
    arr = np.asarray(arr)

    # Number of features the scaler was fit on (should be 100*200=20000)
    if hasattr(scaler, "n_features_in_"):
        n_features = scaler.n_features_in_
    else:  # fallback for very old sklearn
        n_features = scaler.scale_.shape[0]

    if arr.size % n_features != 0:
        raise ValueError(
            f"inverse_scale: input size {arr.size} not divisible by scaler features {n_features}"
        )

    arr2d = arr.reshape(-1, n_features)  # (N, 20000)
    inv2d = scaler.inverse_transform(arr2d)  # (N, 20000)
    return inv2d.reshape(arr.shape)  # back to original shape, e.g. (B,1,100,200)


def main():
    parser = argparse.ArgumentParser()

    parser.add_argument('--model', choices=['srcnn', 'unet', 'fno', 'edsr'], required=True)
    ...
    print(f"Fetched {len(mp_ids)} candidate IDs (show 10):", mp_ids[:10])
    ...
    if len(inputs) < 1:
        ...
        raise RuntimeError(f"No valid samples collected! (success={len(inputs)}, failed={len(errors)})")

    elif len(inputs) < args.limit:

        print(f"Warning: only collected {len(inputs)}/{args.limit} valid samples; training will proceed with fewer.")
    ...
    # 保持你原来的 SmoothL1（最小改动）；如需 RMSE，可改成 criterion = RMSELoss()
    ...
    speed_mpid = args.speed_mpid or ok_ids[0]
